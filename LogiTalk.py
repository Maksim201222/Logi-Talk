...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.label = None
        self.menu_frame = CTkFrame(self, width=30, height=300, fg_color="#808080")
        self.menu_frame.pack_propagate(False)
        self.menu_frame.place(x=0, y=0)
        self.is_show_menu = False
        self.speed_animate_menu = -20

        self.btn = CTkButton(self, text="RIGHT", command=self.toggle_show_menu, width=30,
                             fg_color="#800080", hover_color="#9932CC", text_color="white")
        self.btn.place(x=0, y=0)

        self.chat_field = CTkScrollableFrame(self, fg_color="#808080")
    ...
    def toggle_show_menu(self):
        if self.is_show_menu:
            ...
            self.btn.configure(text='RIGHT')
            self.show_menu()
        else:
            self.is_show_menu = True
            self.speed_animate_menu *= -1
            self.btn.configure(text='LEFT')
            self.show_menu()
    ...
    def save_name(self):
        ...
        if new_name:
            ...
            self.add_message(f"Тепер вас звуть: {self.username}")
    ...
    def open_image(self):
        ...
        try:
            ...
            self.sock.sendall(data.encode())
            pil_img = Image.open(file_name).resize((300, 300))
            self.add_message('', CTkImage(pil_img, size=(300, 300)))
        except Exception as e:
